...
void guardarAlumnos(struct Persona alumnos[], const char *filename);
void cargarAlumnos(struct Persona alumnos[], const char *filename);
...
int main() {
	...
	do {
		...
		printf("6) Guardar alumnos en archivo\n");
		printf("7) Cargar alumnos desde archivo\n");
		printf("8) Salir\n");
		...
		case 6:
			if (datosCargados) {
				guardarAlumnos(Alumnos, "alumnos.bin");
			...
			break;
		case 7:
			cargarAlumnos(Alumnos, "alumnos.bin");
			datosCargados = 1;
...
// Funcion para guardar los datos de los alumnos en un archivo binario
void guardarAlumnos(struct Persona alumnos[], const char *filename) {
	FILE *file = fopen(filename, "wb");
	if (file != NULL) {
		fwrite(alumnos, sizeof(struct Persona), NUM_ALUMNOS, file);
		fclose(file);
	} else {
...
// Funcion para cargar los datos de los alumnos desde un archivo binario
void cargarAlumnos(struct Persona alumnos[], const char *filename) {
	FILE *file = fopen(filename, "rb");
	if (file != NULL) {
		fread(alumnos, sizeof(struct Persona), NUM_ALUMNOS, file);
		fclose(file);
	} else {
